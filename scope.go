...
func ResolvePrimaryKey(value interface{}) string {
    var f reflect.StructField
    t := reflect.TypeOf(value)
    if t.Kind() == reflect.Ptr {
        f = reflect.ValueOf(value).Elem().Type().Field(0)
    } else {
        f = t.Field(0)
    }
    if pkey := f.Tag.Get("primary_key"); len(pkey) > 0 {
        return pkey
    }
    return "id"
}
...
func (db *DB) NewScope(value interface{}) *Scope {
    db.Value = value
    return &Scope{db: db, Search: db.search, Value: value, _values: map[string]interface{}{}, pkey: ResolvePrimaryKey(value)}
}

// New create a new Scope without search information
func (scope *Scope) New(value interface{}) *Scope {
    return &Scope{db: scope.db.parent, Search: &search{}, Value: value, pkey: ResolvePrimaryKey(value)}
}
